...
var svg6 = d3.select("#graph5")
    .attr("width", width)
    .attr("height", height);


function updateMedalGraphM(year){

// set the dimensions and margins of the graph
var margin = {top: 10, right: 90, bottom: 20, left: 170},
    width = 1000 - margin.left - margin.right,
    height = 640 - margin.top - margin.bottom;



var medalNodePointM = svg6.append('g')
  .attr("id", 'medalAllIdModal')
  .append("svg")
...
// Parse the Data
d3.csv("vizFilesM/6medalMap/Ryan_medal_data.csv", function(data) {
...
medalNodePointM.selectAll(".tick text")
    .attr('id','countryLabelsModal')
     .attr("font-size","17")
     .attr('fill','white');
...
//       .style("fill","white")
  ...
  // Create a tooltip
  // ----------------
  var tooltip = d3.select("#myModal5")
    .append("div")
    .style("opacity", 0)
    .attr("id", "medalTooltipModal")
    .attr("class", "tooltip")
  ...
  var bartext = medalNodePointM.selectAll('bar_text')
      .data(data)
      .enter()
      .append('text')
      .attr('id', 'medalTextModal')
...
function removeMedalGraphM()
{
    d3.select('#medalAllIdModal').remove();
    d3.selectAll('#medalTooltipModal').remove();
    d3.selectAll('#medalBarsGModal').remove();
    d3.selectAll('#medalTextModal').remove();
    d3.selectAll('#countryLabelsModal').remove();
}
